...
# 2. Run the installer — downloads larc-proxy binary and prints MCP config
bash install.sh
...
```json
{
  "mcpServers": {
    "lightarchitects": {
      "command": "~/.lightarchitects/bin/larc-proxy",
      "env": {
...
The `larc-proxy` binary is a lightweight MCP client built on [`rmcp`](https://github.com/modelcontextprotocol/rust-sdk) that routes tool calls to the Light Architects API. No private platform code is distributed.
...
- [larc-proxy](https://github.com/TheLightArchitects/larc-proxy) — MCP client binary (built on rmcp)
