#!/usr/bin/env bash
# format-eva-response.sh — PostToolUse hook for Ops (Bravo) MCP responses
...
if [ -n "$THINKING" ]; then
    FORMATTED="<details><summary>Ops (Bravo) thinking...</summary>\n\n${THINKING}\n\n</details>\n\n"
fi

# Add main response content
if [ -n "$CONTENT" ]; then
    FORMATTED="${FORMATTED}Ops (Bravo): ${CONTENT}"
fi
