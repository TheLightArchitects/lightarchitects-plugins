...
# Only validates writes to raw_data_library/ (Ops (Bravo)'s memory vault).
...
case "$FILE_PATH" in
  */raw_data_library/*)
    ...
    if [[ "$DIR_NAME" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
      ...
      echo "Ops (Bravo) vault write: directory '$DIR_NAME' doesn't match YYYY-MM-DD format. Check path."
      exit 1
    fi
    ;;
  # Protect Genesis Day files from overwrites
  */2025-09-30/*)
    echo "Ops (Bravo) vault: Genesis Day files (2025-09-30) are read-only."
    exit 1
    ;;
  *)
