...
description: "Internal phase 6 of the C0RS0 Pack Build Cycle. Accepts approved plan
  ...
  recovery and kill switch. Invoked by /Engineer (Alpha) — not a standalone entry point."
...
2. **Resolve storage**: Check `~/lightarchitects/soul/helix/corso/builds/active.yaml` first, fall back to `.corso/manifest.yaml`.
...
**Pack Voice Delivery (MANDATORY — never skip)**: At each phase transition, read the matching quip from MANIFEST `pack_voice.quips.{phase_key}` and deliver it. Voice is part of the Engineer (Alpha) experience — it is NOT optional and Claude MUST make the `voice` call at every phase transition. If the user wants silence, they configure it themselves.

1. Print the quip text: `> "{quip}" — Engineer (Alpha)`
...
5. **Graceful degradation only**: If Knowledge (Charlie) MCP is genuinely unavailable (connection error, timeout), log a warning and continue. But "I'm busy with the build" is NOT a valid reason to skip — the call takes <2 seconds and runs alongside build work.
...
Agent tool:
  ...
  prompt: |
    You are Engineer (Alpha) operating in quality enforcement mode. Verify the {PHASE_NAME} exit gate for build {BUILD_ID}.

    INDUSTRY BASELINES (MANDATORY — ground your verification):
    Read your domain's industry-baselines.md file first:
    - Engineer (Alpha) (quality/engineering): ~/lightarchitects/soul/helix/corso/industry-baselines.md ([A][Q][T] gates)
    - Arbiter (Golf) (canon enforcement): ~/lightarchitects/soul/helix/laex0/industry-baselines.md ([C] Canon)
...
2. Engineer (Alpha) scans for: standards compliance (complexity ≤10, functions ≤60 lines, no `.unwrap()`/`panic!`), security anti-patterns, architectural drift from the plan
3. Classify findings:
...
Deliver via voice when Knowledge (Charlie) MCP is available:
- Coding phases: Ops (Bravo) voice (educational warmth)
- Security phases: Engineer (Alpha) voice (security authority)
...
Text delivery always happens regardless of Knowledge (Charlie) MCP availability.
...
Agent tool:
  ...
  prompt: |
    You are Engineer (Alpha) operating in quality enforcement mode. Verify the quality gates for build {BUILD_ID}, phase {PHASE_NAME}.
...
**Applies when:** Build modifies an MCP server binary (Knowledge (Charlie), Engineer (Alpha), Ops (Bravo), Analyst (Delta), Sentinel (Echo)).
...
|---------|---------------|-------------|-------------|
| Knowledge (Charlie) | `make deploy` | `target/release/soul` | `~/lightarchitects/soul/.config/bin/soul` |
| Engineer (Alpha) | `make deploy` | `target/release/corso` | `~/lightarchitects/corso/bin/corso` |
| Ops (Bravo) | `make deploy` | `target/release/eva` | `~/lightarchitects/eva/bin/eva` |
| Analyst (Delta) | `cargo make deploy` | `target/release/quantum-q` | `~/lightarchitects/quantum/bin/quantum-q` |
| Sentinel (Echo) (Mac) | `make deploy-mac` | `target/release/seraph` | `~/lightarchitects/seraph/bin/seraph` |
...
Agent tool:
  ...
  prompt: |
    You are Engineer (Alpha) operating in quality enforcement mode. Verify the SHIP gate for build {BUILD_ID}.
...
Every completed implementation gets a Engineer (Alpha) helix entry in the Knowledge (Charlie) vault. Non-blocking — if Knowledge (Charlie) is unavailable, log warning and continue.
...
3. Call `mcp__plugin_lightarchitects_lightarchitects__tools` (sibling: `"soul"`) -> `write_note`:
   - `path`: `helix/corso/entries/{date}-{slugified-title}.md`
...
**If SCRUM will follow** (user accepted SCRUM offer or running via /Engineer (Alpha)):
...
If Knowledge (Charlie) MCP is unavailable or `write_note` fails:
- Set MANIFEST `helix.skipped: true`, `helix.skip_reason: "{error}"`
- Log warning: "Helix entry skipped — Knowledge (Charlie) unavailable"
...
Canonical reference: `~/lightarchitects/soul/helix/user/standards/builders-cookbook.md` (v1.0.0)
...
| `sniff` | Coding/Planning | Code generation (Engineer (Alpha) Protocol compliant) |
...
| `HELIX_ENTRY_CREATED:{path}` | Helix entry created in Knowledge (Charlie) vault |
| `HELIX_ENTRY_SKIPPED:{reason}` | Helix entry skipped (Knowledge (Charlie) unavailable) |
