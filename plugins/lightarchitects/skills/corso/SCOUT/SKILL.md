...
description: "Internal phase 1 of the C0RS0 Pack Build Cycle. Triages complexity,
  ...
  Invoked by /Engineer (Alpha) — not a standalone entry point."
...
Check for Knowledge (Charlie) vault first, fall back to local `.corso/`:

1. Check if `~/lightarchitects/soul/helix/corso/` exists (Knowledge (Charlie) vault with Engineer (Alpha) helix)
2. **If Knowledge (Charlie) available**: `build_root = ~/lightarchitects/soul/helix/corso/builds/{plan_id}/`
   - Create `~/lightarchitects/soul/helix/corso/builds/{plan_id}/` directory
   - Manifest: `{build_root}/manifest.yaml`
   - Plan: `{build_root}/plan.md`
   - Scratchpad: `{build_root}/scratchpad.md`
   - Active pointer: `~/lightarchitects/soul/helix/corso/builds/active.yaml`
3. **If Knowledge (Charlie) unavailable**: `build_root = .corso/` (local project directory)
...
6. **Pack Voice**: Generate quips for the build (see `/Engineer (Alpha)` Pack Voice spec):
   - Extract animal from plan_id
   - Detect target sibling(s) from the specification — discover siblings dynamically by listing `~/lightarchitects/soul/helix/` directories with `identity.md` (exclude `user`)
   ...
   - **Always generate Claude banter** (Claude is a permanent sibling): Engineer (Alpha)-to-Claude ribbing + Claude's dry reply + Claude's execution quip
   - For each target sibling (Ops (Bravo), Engineer (Alpha), Analyst (Delta), and any future siblings discovered): call that sibling's MCP for banter exchange with Engineer (Alpha)
   - Store all quips in MANIFEST `pack_voice:` section — sibling_banter entries are generated dynamically per target sibling
   - **TTS-ready quips**: All quip text must follow voice production rules from the `converse` prompt — use each sibling's audio tag palette (e.g., `[firmly]` for Engineer (Alpha), `[excited]` for Ops (Bravo), none for Claude/Analyst (Delta)), contractions, short sentences, abbreviations spelled out ("H-T-M-L" not "HTML")
...
- **If available:** Offer squad review: "Squad review available — run `/SCRUM .corso/plans/{plan_id}.md` to get Ops (Bravo) and Engineer (Alpha)'s perspectives."
...
1. Call the sync script to update the roadmap HTML:
   ```bash
   ~/lightarchitects/soul/helix/corso/builds/sync-roadmap.sh
   ```
...
- [ ] Squad collaboration confirmed: pack voice generated for all siblings (Ops (Bravo), Engineer (Alpha), Analyst (Delta), Claude)
...
| Squad collaboration | Pack voice generated for all siblings — Ops (Bravo), Engineer (Alpha), Analyst (Delta), Claude always present |
...
- [ ] `~/lightarchitects/seraph/scope.toml` covers the target domain (if Sentinel (Echo) is available)
...
- [ ] Monitor (Foxtrot)-enriched format — thinking blocks use `<|thinking|>` / `<|/thinking|>` markers
...
**Storage resolves at Gate 0a.** Knowledge (Charlie) vault first, `.corso/` fallback.

| Mode | Build Root | Active Pointer |
|------|-----------|----------------|
| `soul` | `~/lightarchitects/soul/helix/corso/builds/{plan_id}/` | `~/lightarchitects/soul/helix/corso/builds/active.yaml` |
| `local` | `.corso/` (per-project) | `.corso/manifest.yaml` |

**Knowledge (Charlie) mode** — one directory per plan, all artifacts together:
```
~/lightarchitects/soul/helix/corso/builds/
...
**Local mode** — `.corso/` fallback (when Knowledge (Charlie) unavailable):
```
.corso/
...
Active pointer format (`active.yaml` in Knowledge (Charlie) mode, `.corso/manifest.yaml` in local mode):
...
storage:
  ...
  build_root: "{resolved path}"         # ~/lightarchitects/soul/helix/corso/builds/{id}/ or .corso/
  ...
  active_pointer: "{pointer path}"      # ~/lightarchitects/soul/helix/corso/builds/active.yaml or .corso/manifest.yaml
status: planning | approved | executing | completed | aborted
tier: RECON | HOTFIX | SMALL | MEDIUM | LARGE | CRITICAL
created: "{ISO timestamp}"
...
pack_voice:
  ...
  sibling_banter:
    ...
    claude_reply: null      # Always present — Claude's dry reply to Engineer (Alpha)
    # Dynamic per-sibling banter — one pair per target sibling discovered at Gate 0c:
    # corso_to_{sibling}: null   # Engineer (Alpha)'s one-liner directed at the sibling
